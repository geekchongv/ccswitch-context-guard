...
function withoutConsoleOutput(action: () => void): void {
...
test("logger rotates the active file into a .1 archive once it exceeds the size cap", () => {
  ...
  withoutConsoleOutput(() => logger.info("the line that tips it over", { marker: "rotation-trigger" }));
...
test("logger keeps rotating across multiple cap crossings", () => {
  ...
  withoutConsoleOutput(() => logger.info("burst-a", { blob: "a".repeat(ROTATION_BYTES + 1024) }));
  assert.ok(!existsSync(archiveFilePath()), "首次写入不应立即归档,归档发生在下一次写入时");

  withoutConsoleOutput(() => logger.info("burst-b", { blob: "b".repeat(ROTATION_BYTES + 1024) }));
  ...
  withoutConsoleOutput(() => logger.info("burst-c", { blob: "c".repeat(ROTATION_BYTES + 1024) }));
